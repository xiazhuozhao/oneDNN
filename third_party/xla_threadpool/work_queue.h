...
class Worker {
 ...
 private:
  ...
  template <typename ParallelTask>
  static std::error_code ExecuteInline(size_t num_tasks,
...
inline WorkQueue::WorkQueue(size_t num_tasks, size_t num_partitions)
  ...
  size_t partition_size = (num_tasks + num_partitions - 1) / num_partitions;
...
inline std::optional<size_t> WorkQueue::Pop(size_t partition_index) {
  assert(partition_index < partitions_.size() && "Invalid partition index");
...
void Worker::ParallelizeWithContext(ParallelizeContext<ParallelTask>* ctx,
                                    uint16_t start_index, uint16_t end_index) {
  assert(start_index < end_index && "Invalid worker index range");
//   std::cout << "ParallelizeWithContext: " << start_index << " " << end_index
//             << std::endl;

  using R = std::invoke_result_t<ParallelTask, size_t>;
  static_assert(std::is_same_v<R, std::error_code> || std::is_void_v<R>,
                "Unsupported parallel task return type");

  auto count_down = [&](size_t count, std::error_code status) {
  ...
  count_down(1, std::error_code());
}

template <typename ParallelTask>
inline std::error_code Worker::ExecuteInline(
    size_t num_tasks, ParallelTask&& parallel_task) {
  using R = std::invoke_result_t<ParallelTask, size_t>;
  static_assert(std::is_same_v<R, std::error_code> || std::is_void_v<R>,
                "Unsupported parallel task return type");
...
template <typename ParallelTask>
inline void Worker::Parallelize(
    const Eigen::ThreadPoolDevice* device,
    CountDownAsyncValueRef<Chain> count_down, size_t num_tasks,
    ParallelTask&& parallel_task) {
...
template <typename ParallelTask>
inline AsyncValueRef<Chain> Worker::Parallelize(
