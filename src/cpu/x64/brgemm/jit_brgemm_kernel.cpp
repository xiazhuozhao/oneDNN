...
void jit_brgemm_kernel_t<Wmm>::store_accumulators_apply_post_ops(dim_t bd_block,
        ...
        for (dim_t ld = 0; ld < ld_block2; ld++) {
            ...
            if (is_single_scale) {
                ...
                if (IMPLICATION(is_tail, isa_has_masks(brg.isa_impl))) {
                    ...
                    // Need to preserve zeros at tail positions.
                    vmulps(vmm_wei_scales, vmm_wei_scales, vmm_tail_mask());
                }
            } else {
                if (IMPLICATION(is_tail, isa_has_masks(brg.isa_impl))) {
            ...
            for (dim_t bd = 0; bd < bd_block; bd++) {
                auto vmm = accm(ld_block2, bd, ld);
                // If there are src scales, conversion happened there.
                if (dq2ps_required && !brg.with_src_scales)
    ...
    for (dim_t ld = 0; ld < ld_block2; ld++) {
        ...
        for (dim_t bd = 0; bd < bd_block; bd++) {
            auto vmm = accm(ld_block2, bd, ld);
            // If there are src or wei scales, conversion happened there.
    ...
    if (brg.with_dst_scales) {
        ...
        for (dim_t bd = 0; bd < bd_block; bd++) {
            auto vmm = accm(ld_block2, bd, ld);
            // See `with_dst_scales` comment for impl details.
