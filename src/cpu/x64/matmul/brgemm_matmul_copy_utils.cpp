...
struct jit_brgemm_matmul_copy_b_bf16_t : public jit_brgemm_matmul_copy_b_t,
    ...
    jit_brgemm_matmul_copy_b_bf16_t(const brgemm_matmul_conf_t *conf)
        ...
        , wei_scales_typesize(sizeof(float))
    ...
    auto load = [this, get_vmm, ncolumns, columns_tail](int blk, int k, int n) {
        ...
        if (utils::one_of(conf_->orig_wei_dt, data_type::s8, data_type::u8,
            ...
            if (req_apply_wei_scales) {
                ...
                uni_vmulps(src_load, src_load, wei_scales_addr);
            }
...
struct jit_brgemm_matmul_copy_b_f32_t : public jit_brgemm_matmul_copy_b_t,
    ...
    jit_brgemm_matmul_copy_b_f32_t(const brgemm_matmul_conf_t *conf)
        ...
        , typesize_wei_scale_(is_src_int4_ ? 2 : 1)
        , wei_scales_typesize_(sizeof(float))
...
private:
    ...
    const size_t typesize_in_, typesize_wei_scale_, wei_scales_typesize_;
...
void jit_brgemm_matmul_copy_b_f32_t<Vmm>::copy_16_x_n_block(
    ...
    auto load = [this, get_vmm, ncolumns](int blk, int k, int n) {
        ...
        auto addr = maybe_EVEX_compress_addr(reg_src,
                (k * src_stride_ + n * typesize_in_) / typesize_wei_scale_);
        ...
        if (req_apply_wei_scales_) {
            ...
            vmulps(maybe_mask(src_vmm, is_tail), src_vmm, wei_scales_addr);
        }
    };
        ...
        if (isa_has_masks(conf_->isa)) {
            ...
            if (is_src_int4_) {
                const auto int4_tail_mask
                        = (1 << (columns_tail / typesize_wei_scale_)) - 1;
...
void jit_brgemm_matmul_copy_b_f32_t<Vmm>::compute_k_loop(int ncolumns) {
    ...
    auto compute_uni_k_loop = [&](int unroll) {
        ...
        add(reg_src, (unroll * src_stride_) / typesize_wei_scale_);
...
void jit_brgemm_matmul_copy_b_transposed_t<Vmm>::maybe_apply_wei_scales(
    ...
    vmulps(vmm, vmm, wei_scales_addr);
}
...
void jit_brgemm_matmul_copy_b_transposed_t<Vmm>::copy_row_x_col(
    ...
    auto load2bf16 = [this, nrows, columns_tail, ncolumns](int i) {
        ...
        auto zmm_src = columns_tail > 0 && ncolumns < req_cvt_bf16_k_blk_step_
                ? src_reg | kTail | T_z
                : src_reg;
        const auto src_offset = (i * src_stride_) / typesize_wei_scale_;
...
void jit_brgemm_matmul_copy_b_transposed_t<Vmm>::compute_K_loop(bool is_N_tail,
    ...
    add(reg_src, (k_blk_step_ * typesize_) / typesize_wei_scale_);
...
void jit_brgemm_matmul_copy_b_transposed_t<Vmm>::compute_N_loop(
    ...
    add(reg_src_base, (n_blk_step_ * src_stride_) / typesize_wei_scale_);
    if (conf_->LDB2 > 0) {
...
struct jit_brgemm_matmul_copy_b_cvt_bf16_t : public jit_brgemm_matmul_copy_b_t,
    ...
    jit_brgemm_matmul_copy_b_cvt_bf16_t(const brgemm_matmul_conf_t *conf)
        ...
        , typesize_wei_scale_(is_src_int4_ ? 2 : 1)
        , src_stride_(
                  (conf->LDB * k_blk_step * typesize_) / typesize_wei_scale_)
...
private:
    ...
    const bool is_src_int4_;
    const dim_t typesize_wei_scale_, src_stride_, tr_src_stride_,
...
void jit_brgemm_matmul_copy_b_cvt_bf16_t<Vmm>::get_wei_scales(const int blk,
    ...
    auto wei_scales_addr0
            = maybe_EVEX_compress_addr(reg_wei_scales, base_offset);
    auto wei_scales_addr1 = maybe_EVEX_compress_addr(reg_wei_scales,
            (k + 1) * wei_scales_N_stride_ + n * wei_scales_typesize_);
    vmovups(zmm_tmp, wei_scales_addr0);
    if (is_k_tail)
        vpxord(vmm_wei_scales1, vmm_wei_scales1, vmm_wei_scales1);
    else
        vmovups(zmm_wei_scales1, wei_scales_addr1);
...
void jit_brgemm_matmul_copy_b_cvt_bf16_t<Vmm>::copy_block(
    ...
    auto load = [this, nrows, ncolumns](int blk, int k, int n) {
        ...
        const dim_t offset = k_blk * src_stride_
                + (n * k_blk_step * typesize_) / typesize_wei_scale_;
        const auto stride = (n_blk_step * typesize_) / typesize_wei_scale_;
...
void jit_brgemm_matmul_copy_b_cvt_bf16_t<Vmm>::generate() {
    ...
    auto compute_K_loop_body
            = [&](const reg64_t &reg_K, int ncolumns, bool zeropad) {
                  ...
                  if (req_apply_wei_scales_)
                      add(reg_wei_scales,
                              k_unroll * k_blk_step * wei_scales_N_stride_);
