    ...
    parallel_nd(batch, outD, outH, outW,
            [&](dim_t mb, dim_t od, dim_t oh, dim_t ow) {
                ...
                while (oc < (size_t)channels) {
                        ...
                        for (int id = id_start; id < id_end; ++id) {
                            for (int ih = ih_start; ih < ih_end; ++ih) {
                                for (int iw = iw_start; iw < iw_end; ++iw) {
                                    ...
                                    const dnnl::impl::float16_t *src_ptr
                                            = &src[src_spatial_base + oc];
                                    vfloat16m1_t vsrc_f16
                                            = __riscv_vle16_v_f16m1(
                                                    (const _Float16 *)src_ptr,
                                                    vl);
                    ...
                    // vout_f32 = postops_handler.apply(vout_f32, vl);

                    vfloat16m1_t vout_f16
                            = __riscv_vfncvt_f_f_w_f16m1(vout_f32, vl);

                    __riscv_vse16_v_f16m1(
                            (_Float16 *)&dst[dst_spatial_base + oc], vout_f16,
                            vl);
                    oc += vl;
                }
            });
    ...
    parallel_nd(batch, outD, outH, outW,
            [&](dim_t mb, dim_t od, dim_t oh, dim_t ow) {
                ...
                while (oc < (size_t)channels) {
                    ...
                    if (count_scalar == 0.0f) {
                        vfloat32m2_t vzero_f32
                                = __riscv_vfmv_v_f_f32m2(0.0f, vl);
                        // vzero_f32 = postops_handler.apply(vzero_f32, vl);
                        vfloat16m1_t vout_f16
                                = __riscv_vfncvt_f_f_w_f16m1(vzero_f32, vl);
                        __riscv_vse16_v_f16m1(
                                (_Float16 *)&dst[dst_spatial_base + oc],
                                vout_f16, vl);
                    } else {
                        vfloat32m2_t vscale_f32 = __riscv_vfmv_v_f_f32m2(
                                1.0f / count_scalar, vl);
                        vfloat32m2_t vout_f32 = __riscv_vfmul_vv_f32m2(
                                vsum_f32, vscale_f32, vl);
...
riscv_nhwc_pooling_fwd_t<data_type>::riscv_nhwc_pooling_fwd_t(const pd_t *apd)
    : primitive_t(apd) {}

template <>
status_t riscv_nhwc_pooling_fwd_t<data_type::f32>::execute_forward(
    ...
    src_raw = (const uint8_t *)src_raw
            + src_d.off_l(0) * types::data_type_size(data_type::f32);
    dst_raw = (uint8_t *)dst_raw
            + dst_d.off_l(0) * types::data_type_size(data_type::f32);
    ...
    if (is_max_pool) {
        MaxPooling_f32(src_raw, dst_raw, MB, C, OD, OH, OW, ID, IH, IW, KD, KH,
                KW, SD, SH, SW, padF, padT, padL, postops_handler);
    } else if (is_avg_pool_exclude) {
        AvgPoolingExcludePadding_f32(src_raw, dst_raw, MB, C, OD, OH, OW, ID,
                IH, IW, KD, KH, KW, SD, SH, SW, padF, padT, padL,
                postops_handler);
    } else if (is_avg_pool_include) {
        AvgPoolingIncludePadding_f32(src_raw, dst_raw, MB, C, OD, OH, OW, ID,
                IH, IW, KD, KH, KW, SD, SH, SW, padF, padT, padL,
                postops_handler);
    } else {
...
#if defined(DNNL_RISCV_USE_ZVFH_INTRINSICS)
template <>
status_t riscv_nhwc_pooling_fwd_t<data_type::f16>::execute_forward(
        const exec_ctx_t &ctx) const {
    ...
    src_raw = (const uint8_t *)src_raw
            + src_d.off_l(0) * types::data_type_size(data_type::f16);
    dst_raw = (uint8_t *)dst_raw
            + dst_d.off_l(0) * types::data_type_size(data_type::f16);
    ...
    if (is_max_pool) {
        MaxPooling_f16(src_raw, dst_raw, MB, C, OD, OH, OW, ID, IH, IW, KD, KH,
                KW, SD, SH, SW, padF, padT, padL, postops_handler);
    } else if (is_avg_pool_exclude) {
        AvgPoolingExcludePadding_f16(src_raw, dst_raw, MB, C, OD, OH, OW, ID,
                IH, IW, KD, KH, KW, SD, SH, SW, padF, padT, padL,
                postops_handler);
    } else if (is_avg_pool_include) {
        AvgPoolingIncludePadding_f16(src_raw, dst_raw, MB, C, OD, OH, OW, ID,
                IH, IW, KD, KH, KW, SD, SH, SW, padF, padT, padL,
                postops_handler);
    } else {
        return status::unimplemented;
...
template struct riscv_nhwc_pooling_fwd_t<f32>;

#if defined(DNNL_RISCV_USE_ZVFH_INTRINSICS)
template struct riscv_nhwc_pooling_fwd_t<f16>;
#endif
