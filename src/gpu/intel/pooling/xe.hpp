...
#ifndef GPU_INTEL_POOLING_XE_HPP
#define GPU_INTEL_POOLING_XE_HPP
...
struct xe_pooling_fwd_t : public gpu_primitive_t {
    ...
    status_t init(impl::engine_t *engine) override {
        ...
        CHECK(create_kernel(engine, &kernel_, "xe_pooling_fwd", kernel_ctx));
...
struct xe_pooling_bwd_t : public gpu_primitive_t {
    ...
    status_t init(impl::engine_t *engine) override {
        ...
        CHECK(create_kernel(engine, &kernel_, "xe_pooling_bwd", kernel_ctx));
        if (!kernel_) return status::runtime_error;
