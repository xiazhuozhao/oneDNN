...
const std::map<pk_impl_key_t, std::vector<impl_list_item_t>>
    ...
    {{forward}, {
        GPU_INSTANCE_INTEL(intel::conv::gen_convolution_fwd_t)
        GPU_INSTANCE_INTEL(intel::conv::xe_wino_convolution_fwd_t)
        GPU_INSTANCE_INTEL_REF(intel::conv::ref_convolution_fwd_t)
        GPU_INSTANCE_INTEL_EXPERIMENTAL(intel::conv::v2::gen_convolution_fwd_t)
    ...
    {{backward_data}, REG_BWD_D_PK({
        GPU_INSTANCE_INTEL(intel::conv::gen_convolution_bwd_data_t)
        GPU_INSTANCE_INTEL_REF(intel::conv::ref_convolution_bwd_data_t)
        GPU_INSTANCE_INTEL_EXPERIMENTAL(intel::conv::v2::gen_convolution_bwd_data_t)
    ...
    {{backward_weights}, REG_BWD_PK({
        GPU_INSTANCE_INTEL(intel::conv::gen_convolution_bwd_weights_t)
        GPU_INSTANCE_INTEL_REF(intel::conv::ref_convolution_bwd_weights_t)
        GPU_INSTANCE_INTEL_EXPERIMENTAL(intel::conv::v2::gen_convolution_bwd_weights_t)
        ...
        if (experimental::use_gpu_conv_v2()) {
            for (auto &kv : list_map) {
                ...
                int fwd_idx = impl_list_item_t::find<
                        intel::conv::v2::gen_convolution_fwd_t::pd_t>(&list[0]);
                int bwd_d_idx = impl_list_item_t::find<
                        intel::conv::v2::gen_convolution_bwd_data_t::pd_t>(
                        &list[0]);
                int bwd_w_idx = impl_list_item_t::find<
                        intel::conv::v2::gen_convolution_bwd_weights_t::pd_t>(
                        &list[0]);
